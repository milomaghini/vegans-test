...
public class JsonPlaceholderService implements JsonPlaceholderServiceRemote {
	...
	public String getPost(int id) throws IOException {			
		...
		Post res = responseToObj(response, new TypeReference<Post>() {});
	...
	public String getComments(int id) throws IOException {
		...
		Request request = new Request.Builder()
			      ...
			      .build();
		Response response = client.newCall(request).execute();
		List<Comments> res = responseToObj(response, new TypeReference<List<Comments>>() {});
		return om.writeValueAsString(res);
	}
	
	private <T> T responseToObj(Response response, TypeReference<T> type) throws IOException {
		T res = null;
		if(response.isSuccessful() && response.body() != null && response.code() == 200) {
			String body = response.body().string();
			res = om.readValue(body, type);
		}
		return res;
	}
